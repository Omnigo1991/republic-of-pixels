import { PixelRaten } from "@/components/PixelRaten";
import { GLAS } from "./Bausteine";

// Abschluss der Startseite wie im Entwurf (Tim, 22.08.2026): links das
// Pixel-Raten in einer Glaskachel, rechts der Newsletter als einzige
// Farbfläche der Seite - beide gleich hoch, ohne eigene Sektionstitel.
export function SchlussKacheln() {
  return (
    <div className="mt-[18px] grid gap-[18px] lg:grid-cols-[3fr_2fr]">
      <div className={`${GLAS} raten-kachel overflow-hidden rounded-[26px] px-[30px] py-7`}>
        <PixelRaten />
      </div>

      <div className="flex flex-col justify-center rounded-[26px] bg-[linear-gradient(120deg,rgba(2,240,209,0.92)_0%,rgba(2,240,209,0.84)_46%,rgba(255,46,151,0.84)_100%)] px-8 py-[30px]">
        <div className="text-[24px] font-bold leading-[1.15] text-[#0C0B1A]">
          Keine Nebenquests.
          <br />
          Nur die besten News.
        </div>
        <p className="mt-2 text-[14.5px] text-[#0C0B1A]">Täglich das Wichtigste in deinem Postfach.</p>
        <div className="mt-4">
          <a
            href="/#newsletter"
            className="inline-block rounded-full bg-[#0C0B1A] px-5 py-2.5 text-[14px] font-semibold text-white"
          >
            Abonnieren
          </a>
        </div>
      </div>
    </div>
  );
}
